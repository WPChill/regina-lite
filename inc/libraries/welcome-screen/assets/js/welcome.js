var welcomeScreenFunctions = {
    ...
    jQuery( '.epsilon-ajax-button' ).on('click', function( e ) {
      ...
      if ( importThis[ 'plugins' ].length ) {
        jQuery( document ).on( 'epsilon-all-plugins-imported', function() {
          self._importContent( importThis[ 'regina_options' ], container );
        } );
      } else {
        self._importContent( importThis[ 'regina_options' ], container );
      }
  ...
  activatePlugin: function() {
    ...
    jQuery( activateButtonSlug ).on( 'click', function( e ) {
      ...
      jQuery.ajax( {
        beforeSend: function() {
          self.replaceWith( '<a class="button updating-message">' + reginaWelcomeScreenObject.activating_string + '...</a>' );
        },
        async: true,
        type: 'GET',
        dataType: 'html',
        ...
        success: function( response ) {
          ...
          if ( ! actions.length ) {
            jQuery( '#plugin-filter' ).append( '<span class"hooray">' + reginaWelcomeScreenObject.no_actions + '</span>' );
          }
    ...
    jQuery( document ).on( 'wp-plugin-install-success', function( response, data ) {
      ...
      if ( activateButton.length && ( jQuery( 'body' ).hasClass( reginaWelcomeScreenObject.body_class ) || jQuery( 'body' ).hasClass( 'wp-customizer' ) ) ) {

        jQuery.ajax( {
          beforeSend: function() {
            activateButton.replaceWith( '<a class="button updating-message">' + reginaWelcomeScreenObject.activating_string + '...</a>' );
          },
          async: true,
          type: 'GET',
          dataType: 'html',
...
jQuery( document ).ready( function() {
  ...
  welcomeScreenFunctions.fixHomepage();
} );
